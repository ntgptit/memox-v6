...
void main() {
  ...
  const today = LocalDay(2026, 7, 26);

  StreakProjection project(List<LocalDay> days, {LocalDay at = today}) =>
      policy.project(days: days, effectiveDay: at);

  test('empty history is 0/0 with no last day', () {
    final result = project(<LocalDay>[]);
  ...
  test('a single day today is a streak of one', () {
    final result = project(<LocalDay>[today]);
  ...
  test('consecutive days accumulate', () {
    final result = project(<LocalDay>[
      const LocalDay(2026, 7, 24),
      const LocalDay(2026, 7, 25),
      today,
    ]);
  ...
  test('a one-day gap closes the run', () {
    ...
    final result = project(<LocalDay>[const LocalDay(2026, 7, 24), today]);
  ...
  test('a broken current run leaves longest intact', () {
    final result = project(<LocalDay>[
      const LocalDay(2026, 7, 1),
      const LocalDay(2026, 7, 2),
      const LocalDay(2026, 7, 3),
      const LocalDay(2026, 7, 4),
      const LocalDay(2026, 7, 20),
    ]);

    expect(result.current, 0, reason: 'the 20th is long past the grace window');
    expect(result.longest, 4);
    expect(result.lastQualifiedDay, const LocalDay(2026, 7, 20));
  });

  test('yesterday still counts, the day before does not', () {
    // `metrics-v1`: consecutive days "ending today, or ending yesterday when
    // today has no qualifying event yet. A gap before yesterday yields zero."

    expect(project(<LocalDay>[const LocalDay(2026, 7, 25)]).current, 1);
    expect(project(<LocalDay>[const LocalDay(2026, 7, 24)]).current, 0);
  });
  ...
  test('a future record is dropped and reported, not counted', () {
    final result = project(<LocalDay>[
      const LocalDay(2026, 7, 25),
      today,
      const LocalDay(2026, 7, 27),
    ]);

    expect(result.current, 2);
    expect(result.lastQualifiedDay, today);
    expect(result.issues, hasLength(1));
    expect(result.issues.single.day, const LocalDay(2026, 7, 27));
  });
  ...
  test('runs cross month and year boundaries', () {
    final result = project(<LocalDay>[
      const LocalDay(2025, 12, 30),
      const LocalDay(2025, 12, 31),
      const LocalDay(2026, 1, 1),
    ], at: const LocalDay(2026, 1, 1));
  ...
  test('the projection is deterministic across input orderings', () {
    final ascending = project(<LocalDay>[
      const LocalDay(2026, 7, 24),
      const LocalDay(2026, 7, 25),
      today,
    ]);
    final descending = project(<LocalDay>[
      today,
      const LocalDay(2026, 7, 25),
      const LocalDay(2026, 7, 24),
    ]);
  ...
  test('reading does not mutate the records it was given', () {
    final days = <LocalDay>[today, const LocalDay(2026, 7, 24)];
    project(days);

    expect(days, <LocalDay>[today, const LocalDay(2026, 7, 24)]);
  });

  test('the projection carries its formula version', () {
    expect(
      project(<LocalDay>[today]).formulaVersion,
  ...
  group('LocalDay', () {
    test('next rolls over months, years and leap days', () {
      expect(const LocalDay(2026, 1, 31).next, const LocalDay(2026, 2, 1));
      expect(const LocalDay(2026, 12, 31).next, const LocalDay(2027, 1, 1));
      expect(const LocalDay(2024, 2, 28).next, const LocalDay(2024, 2, 29));
    });

    test('a local date drops the time of day', () {
      expect(
        LocalDay.of(DateTime(2026, 7, 26, 23, 59)),
        LocalDay.of(DateTime(2026, 7, 26, 0, 1)),
      );
    });
  });
}
