...
| KIT-31-04 | `_shared/DeckActionsSheet.jsx:15` (Delete deck ở cuối + `danger`, tách khỏi row khác); `kit-helpers.jsx:150` (Dialog actions row gap `--memox-space-3`), `kit-helpers.jsx:128,133` (MenuItem `danger` tint `--memox-error`) | PASS | Ghost cancel (trái) + danger confirm (đỏ, phải), 12px gap, cả hai ≥48px; destructive ở MenuItem là dòng cuối, đỏ, tách biệt. |
