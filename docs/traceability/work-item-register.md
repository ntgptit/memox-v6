...
| `5.1.2` | **Reopened** | DG-01 | create-language-pair.md, 3.12 Mx set | `AC-WBS-5.1.2-01`; `TEST-WBS-5.1.2-01` | `docs/wbs/implementation-packets/WBS-5.1.2-first-run-language-ui.md` | First feature screen: route + draft/save viewmodels + searchable sheet + 5 widget tests over the real graph; first-run redirect boundary → 5.7 (2026-07-19) **Reopened 2026-07-20** for `MX-VIS-005`: the kit draws an inline error under the language select and `MxSelectRow` has no error slot, so the state is unreachable. Everything else in the item stands. |
...
| `5.2.1` | **Reopened** | DG-01 | deck README §0, create/move-deck rules | `AC-WBS-5.2.1-01`; `TEST-WBS-5.2.1-01` | `docs/wbs/implementation-packets/WBS-5.2.1-deck-domain.md` | Derivation (empty/parent/leaf, mixed→corruption) + deck-name normalization + NEW `deck-pair-mismatch` triggers (cross-pair nesting gap closed) + contentCounts port; 6 tests; snapshot re-exported (2026-07-19) **Reopened 2026-07-20** for `MX-VIS-014`: no deck-name length rule exists anywhere in domain or presentation, so the too-long state cannot render. |
...
| `5.3.1` | In progress (A Done) | DG-01 | create/resolve-duplicate-flashcard.md, VAL-001 | `AC-WBS-5.3.1-01`; `TEST-WBS-5.3.1-01` | `docs/wbs/implementation-packets/WBS-5.3.1-flashcard-domain.md` | Child A: normalized_term column + duplicate-candidate query + CreateFlashcardUseCase (pre-commit candidates, keep-both, kept-id) + 7 tests (2026-07-19); children B (child content), C (lifecycle/evidence) pending |
