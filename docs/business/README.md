...
Cross-object validation and normalization are indexed in
[`validation-and-normalization-catalog.md`](./validation-and-normalization-catalog.md).
...
## 0. Product decisions fixed for v1
...
| BD-007 | V1 locales are English and Vietnamese; contracts are RTL-ready, runtime RTL is deferred |

These decisions resolve previous Business↔Design gates. Any change requires an explicit superseding decision and coordinated Business/Design/Architecture/WBS update; a consuming document may not silently choose another rule.
...
| Navigation | [navigation](./navigation/README.md) | Stable destinations, first-launch/platform back/deep-link guards | Business data hoặc feature mutation |
...
- Navigation áp prerequisite/identity guards và recovery destination; owning object vẫn quyết định eligibility/mutation.
