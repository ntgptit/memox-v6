...
Flow này sở hữu submit answer cho các Study stages, persist Attempt và advance checkpoint. Learning Progress sở hữu scheduling state.
...
- Mastery retry round thuộc stage hiện tại và khác với persistence Retry cũng như một future Relearn session.
...
| SRS Binary Review | Chọn Remembered hoặc Relearn | `correct` | `wrong` |
...
flowchart TD
    ...
    M --> A
    L -- "Không" --> N["Commit terminal outcomes"]
    N --> P["Finalize current session"]
```
...
- Match `almost` và Fill comparison phải lần lượt tuân [SM-MATCH-v1](../../decision-tables/match-outcomes.md) và [SM-FILL-v1](../../decision-tables/fill-answer-normalization.md); UI không tự định nghĩa heuristic.
- Outcome không đạt được thêm vào `nextRoundFailedCardIds` theo Card identity; thêm lặp lại vẫn chỉ tạo một entry.
- Với SRS-active Card, mọi committed `wrong`, `almost` hoặc Recall timeout là sticky lapse evidence cho terminal grade. Retry round giúp Card đạt mastery để kết thúc session nhưng không xóa lapse; terminal grade vẫn là `wrong` và SRS hạ đúng một box theo [SRS Policy v1](../learning-progress/srs-8-box-policy.md).
...
| Missing prompt | `This card is no longer available. Continue with the next card.`; skip audit-only, không Attempt/grade/schedule |
...
- Content mutation race tuân [ST-CONTENT-CHANGE-v1](../../decision-tables/study-session-content-changes.md); current prompt delete bị block, missing non-current Card skip không ảnh hưởng accuracy denominator.
- Sáu strategy canonical states và answer-save-error parity dưới 3% mỗi theme.
