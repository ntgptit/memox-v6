...
- Start chỉ thành công khi scope, `sessionType`, mode khi cần và eligible Card set vẫn hợp lệ.
- Snapshot cố định Deck id, base Card ids, `sessionType`, mode plan, scope, effective preferences, start time, `shuffleVersion` và session shuffle seed material.
...
```mermaid
flowchart TD
    A["Start session"] --> B["Revalidate Deck, scope, session type, mode plan, cards"]
    B --> C{"Active session?"}
...
- Objective: bắt đầu đúng một session với scope/session type/mode plan đã xác nhận.
...
| Session-type/mode-plan minimum not met | Giữ picker; chọn scope/type/mode hợp lệ |
| Guess không có ít nhất 5 meaning khác nhau | Chặn Start; yêu cầu thêm Card hoặc chọn scope khác |
| `newLearning` thiếu bất kỳ stage bắt buộc | Chặn Start; không hạ xuống single-mode |
| `practice` chưa chọn mode | Chặn Start; giữ Mode Picker |
| `dueReview` có zero due | Caught-up; không tạo session rỗng |
...
Mode Picker chỉ cập nhật selection. Session chỉ được tạo sau explicit `Start session`; chọn tile, keyboard focus hoặc deep-link preload không tự submit.
...
- Snapshot chỉ chứa eligible Cards và đáp ứng session-type/mode-plan minimum.
...
- Session snapshot tuân mọi row của [ST-SESSION-TYPE-v1](../../decision-tables/study-session-types.md).
