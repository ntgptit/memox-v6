...
- `guess-meaning-normalize-v1` = Unicode NFC → locale-aware case fold → trim outer whitespace → collapse internal Unicode whitespace. Giữ diacritics/punctuation; không translate/transliterate. Eligibility và question builder phải dùng cùng version.
...
- Distinctness key là output của `guess-meaning-normalize-v1`; raw strings khác nhau nhưng cùng key chỉ được tính một meaning.
...
- Eligibility và option construction dùng cùng `guess-meaning-normalize-v1`; normalization version được lưu trong snapshot/evidence.
