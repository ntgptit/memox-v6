...
- Canonical classification bắt buộc theo [SM-MATCH-v1](../../decision-tables/match-outcomes.md): `almost` chỉ khi pair ids khác nhưng normalized display meanings bằng nhau; mọi mismatch khác là `wrong`.
...
- `almost` không được suy từ gesture distance, animation hoặc response time. Normalization dùng NFC, locale-aware case fold và whitespace normalization; giữ dấu/punctuation.
...
- Mỗi row `SM-MATCH-*` có automated test dẫn exact ID.
