...
Executable contract [SRS8-v1](../../decision-tables/srs-8-box-v1.md) bao phủ activation, toàn bộ transition Box 1..8, sticky-wrong, equality boundary, hidden/deleted, reset, practice, unknown policy, idempotency và concurrency. Tài liệu này sở hữu policy semantics; decision table sở hữu exact row IDs dùng cho test. Mỗi row bắt buộc có ít nhất một test và test phải dẫn exact ID.

Migration v1 là initial assignment: Progress mới lưu `policyId=leitner-8-box-v1`. Không reinterpret history dưới cùng ID. Policy tương lai bắt buộc có ID mới, bảng mapping từng source box/state, dry-run impact counts, transaction/rollback và fixture tests trước release.
