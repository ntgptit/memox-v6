...
| MX-VIS-063 | Match | Selected | `match-mode--selected` | **Playwright PASS — 1.65% light / 2.96% dark** (resume into Match, tap the second left-hand tile). The kit selects `love`; both columns are seeded from the kit's own pairs and `love` lands second here too, so the highlight falls in the same row. **Dark sits 0.04pp under the gate** — the selected tile's tinted fill is the largest coloured area either theme measures, and dark amplifies it |
