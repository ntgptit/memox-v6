...
| MX-VIS-078 | Study session | Answer save error | `study-session--answer-save-error` | **Measured — 21.49% light / 21.42% dark. Blocked on the shared confirm composite.** The state itself is right: the dialog reports the failure `int-40` made visible, over the Fill stage that raised it, with the kit's `sync_problem` glyph and its `Back` + `Retry` pair. What the capture found is one layer down — this app has a single `MxDialog` doing the work of the kit's *two* dialog helpers. The kit's icon `Dialog` is a centred column (icon tile, then title and text, then a full-width actions row); `MxDialog` is its `FormDialog` shape (left-aligned title, body, right-aligned wrapped actions), which is right for create/rename and wrong here. So the confirm renders title-above-icon, left-aligned, with stacked natural-width buttons. Six sites share that composite, so the fix belongs to the shared widget and its checklist rather than to this state |
