...
| MX-VIS-038 | Deck detail — parent branch | Error | `subdeck-list--error` | Composition built, **not yet measured**. The branch now renders the kit's centred failure — `cloud_off` tile, `Couldn’t load decks`, the shared body, a Retry that re-invalidates the child stream — instead of the async builder's default inline banner, and the app-bar search glyph it was waiting on shipped earlier. Needs a `?fixture=` that breaks the child read plus a spec before a ratio exists. **Measurement attempted twice on 2026-07-25 and backed out; the cause is now narrowed to the parity build itself.** What is proven: Riverpod surfaces the failure correctly — a `ProviderContainer` probe over an erroring child stream reads `AsyncError` — and the screen renders it correctly, since a widget test overriding `deckChildrenProvider` with the same broken stream shows `Couldn’t load decks`, the body and `Try again`. In the parity web build the same override leaves the screen on its loading skeleton, which is excluded from semantics, so the spec sees an app bar over an empty body. That holds for an override on `OpenDeckUseCase` and on the presentation provider, for `Stream.error` and for a `StreamController` that stays open, and the overrides do take effect (the real children never load). The remaining suspect is the release-web path — most likely the bootstrap error zone absorbing the stream error before the provider settles. Five passes did not close it; a spec that fails on its own setup is noise, so none was landed |
| MX-VIS-039 | Deck detail — parent branch | Not found | `subdeck-list--not-found` | Composition built, **not yet measured**. Now the kit's `folder_off` warning tile with `This deck no longer exists` and a single Back to Library action, and the app bar drops its search and overflow actions — the kit's reason being that there is nothing left to search or configure once the deck is gone. Reaching it through a real journey is the open question: the deck has to disappear while open, and a deep link is banned by §6.6 |
...
| MX-VIS-044 | Deck detail — leaf branch | Error | `flashcard-list--error` | Composition built, **not yet measured**. Same centred failure as `MX-VIS-038` with the leaf's own title (`Couldn’t load cards`), replacing the inline banner. Needs a fixture that breaks the card read plus a spec. Blocked on the same unresolved fixture mechanics as `MX-VIS-038` |
