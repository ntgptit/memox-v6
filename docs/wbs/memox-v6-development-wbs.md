...
| MX-VIS-036 | Deck detail — parent branch | Loaded | `subdeck-list--loaded` | Blocked, but two of its four gaps are closed and it is still **unmeasured** (no fixture or spec exists yet, so there is no ratio to quote). The breadcrumb shipped with `6.2`; the branch now renders the kit's `DECKS` section label over the same `MxDeckCard` rows the Library root uses, carrying the `5.4.2` due/new counters through a new `watchChildDeckSummaries` read model. Remaining: the app-bar search glyph (`5.x`). The per-row play affordance shipped on 2026-07-25 after one reverted attempt: a nested control turns the deck row from a leaf semantics node into a parent one, which moves the row's accessible name from rendered text onto `aria-label`, and six specs were locating decks with `getByText(deckName)`. Those assertions now match the accessible name — what a screen reader announces, and what the rest of this harness already asserts — so the row stays addressable however it is composed. Parity held at 34/36 across the change |
