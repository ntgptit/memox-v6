...
| MX-VIS-036 | Deck detail — parent branch | Loaded | `subdeck-list--loaded` | Blocked, but two of its four gaps are closed and it is still **unmeasured** (no fixture or spec exists yet, so there is no ratio to quote). The breadcrumb shipped with `6.2`; the branch now renders the kit's `DECKS` section label over the same `MxDeckCard` rows the Library root uses, carrying the `5.4.2` due/new counters through a new `watchChildDeckSummaries` read model. **All four chrome gaps are now closed, and it is still unmeasured** — no fixture or spec exists for it, so there is no ratio to quote. The breadcrumb shipped with `6.2`; the branch renders the kit `DECKS` section label over the same `MxDeckCard` rows the Library root uses, carrying the `5.4.2` due/new counters; each row has the kit's study bolt; and the app bar carries search. Two of those cost more than they looked: the bolt turns a deck row from a leaf semantics node into a parent one, which moves the row's accessible name from rendered text onto `aria-label` and broke six specs locating decks with `getByText` (they now match the accessible name); and the search entry is library-wide, because narrowing to a deck is the Deck picker inside search's own filter sheet (`filter-search-results.md` §3), which is not built. Adding search moved the states whose shots contain it by **+0.02pp** — neither an improvement nor a regression, which is worth recording so the glyph is not credited with a gain it did not produce. Writing this row's fixture and spec is the next step |
| MX-VIS-037 | Deck detail — parent branch | Loading | `subdeck-list--loading` | **Blocked on `MX-VIS-036` — measured 7.08% light / 8.86% dark.** The shot and the kit `SubdeckList.jsx` loading branch disagree, and the shot wins (§6.4 VP-06). The JSX renders a full-width 40px filter pill over an `MxList`; the **shot** shows a short half-width breadcrumb placeholder, a real `DECKS` section label, then deck-row cards. That is a skeleton of the kit's *loaded* parent chrome — breadcrumb, section label, `MxDeckCard` rows — none of which the app's parent branch has (it renders a summary caption and plain folder rows, which is exactly why `MX-VIS-036` is Blocked). Building the placeholder to match would promise a layout the loaded screen does not deliver, so this defers **with** `MX-VIS-036` rather than being fixed on its own. The number rose from 5.54% when the shared `space-4` body inset was corrected: right for the screen, further from this particular shot |
