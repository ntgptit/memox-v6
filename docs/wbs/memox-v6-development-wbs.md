...
| MX-VIS-049 | Card Editor | Create | `flashcard-editor--create` | **Playwright PASS — 1.20% light / 1.63% dark** (fresh app launch → first-use Create Deck A→K → user opens Empty Deck → Create Flashcard A→J; capture at C, PASS only after Save returns to Leaf list with the committed Card visible) |
