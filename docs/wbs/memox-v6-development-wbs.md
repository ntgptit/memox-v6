...
| MX-VIS-036 | Deck detail — parent branch | Loaded | `subdeck-list--loaded` | **Measured — 20.26% light / 28.84% dark. FAIL (gate ≤3%).** No longer "believed close": a fixture seeds five children whose card totals sum to the shot's `217 cards` and whose due counts sum to `23 due`, so the aggregate header is reproduced by data rather than asserted. The four chrome gaps closed earlier (breadcrumb, section label + counters, row bolt, search) were real but not what dominates. Five differences remain, and three of them are decisions rather than polish: **(1)** the app hides the breadcrumb for a root deck by design (`a root deck shows nothing — the app-bar back is its up-navigation`) while the shot draws `Library › Korean TOPIK I` — a design↔implementation divergence that needs a ruling; **(2)** the screen carries a full-width **Study** CTA the shot does not, and it is not redundant with the new per-row bolt because it studies the whole subtree, so removing it drops a capability; **(3)** the kit puts create-deck in a **FAB** where the screen uses a full-width button. The other two are mechanical: `MxSectionHeader` stacks title over caption where the kit sets one line (`DECKS · 217 cards · 23 due`), and every row draws the same glyph because `Deck` has no icon field — a schema change the deck spec does not yet describe |
