...
| MX-VIS-038 | Deck detail — parent branch | Error | `subdeck-list--error` | Composition built, **not yet measured**. The branch now renders the kit's centred failure — `cloud_off` tile, `Couldn’t load decks`, the shared body, a Retry that re-invalidates the child stream — instead of the async builder's default inline banner, and the app-bar search glyph it was waiting on shipped earlier. Needs a `?fixture=` that breaks the child read plus a spec before a ratio exists. **Measurement attempted 2026-07-25 and backed out:** an `openDeckUseCaseProvider` override returning `Stream.error` never reaches the widget in the parity build — the screen stays on the loading skeleton, which is semantically invisible, so the spec sees an empty body rather than the error. The same override mechanism works for the pending-stream fixtures (`MX-VIS-037`/`043`), so the difference is in how an erroring stream propagates there, not in the override itself. Unresolved after three passes; a spec that fails on its own setup is noise, so none was landed |
