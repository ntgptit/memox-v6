...
| MX-VIS-084 | Search | Results | `search--results` | **Measured 2026-07-28 — 1.95% light PASS / 3.20% dark FAIL** (`int-105`). Registered and measured in the same round: the screen had no row while four rounds of changes landed on it, so this is a first number rather than a regression. The residual is not composition — light passes at the same layout. The dark diff is dominated by fixture-versus-shot content (the shot lists three Korean-term results; the fixture's query matches one English card) plus a trailing element on each kit row the app does not draw. Ratcheted in `known-gaps.json`; closing it needs a fixture shaped like the shot and a decision on that trailing element |
