...
| MX-VIS-057 | Card Editor | Submit error | `flashcard-editor--submit-error` | **Light PASS 1.58% / dark FAIL 4.92%** (create journey → Save against a write path that throws; the draft survives). Was 3.36 / 9.99 before the banner was corrected to the kit's untitled inline shape — the call site passed the message as `title`, which is the stacked decision layout, not the recoverable-failure one. The dark residual is colour, not composition: the same tree passes in light. **Both this shot and `--submitting` are drawn in the kit's *edit* variant** ("Edit card"), while these journeys are create ("New card"); measured at ~1% on `MX-VIS-056`, so it is not the dark blocker but it does mean neither shot can reach 0 from a create journey |
