...
| MX-VIS-069 | Today | Loaded | `dashboard--loaded` | Composition partial — greeting and the Daily-goal card done (2026-07-26); the Today inline strip, Continue-studying section and Recent-decks list are outstanding (WBS 5.7.2) |
| MX-VIS-070 | Today | Not studied | `dashboard--not-studied` | Composition partial — same sections as `MX-VIS-069`, plus the kit's "you haven't studied today" note |
| MX-VIS-071 | Today | Goal met | `dashboard--goal-met` | Composition partial — the Daily-goal card renders its met state (`All 20 cards · goal complete`, bar at 100%); the kit's celebration note and the surrounding sections are outstanding |
| MX-VIS-072 | Today | Streak reset | `dashboard--streak-reset` | Composition partial — the streak projection reports a broken run as `current = 0` and the goal card renders; the kit's streak-reset note is outstanding |
| MX-VIS-073 | Today | Caught up | `dashboard--caught-up` | Composition partial — the all-caught-up message exists; the greeting, strip and deck rows around it do not |
