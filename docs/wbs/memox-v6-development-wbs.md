...
| MX-VIS-078 | Study session | Answer save error | `study-session--answer-save-error` | **Measured — 24.50% light / 24.92% dark.** The dialog is the kit's icon `Dialog` composite (`int-54`); the residual is everything around it: the kit stages the shot over Stage 5 - Fill at 21/25 on the card 'school' while the fixture is a five-card session on its first card, the app bar is `int-5`'s divergence, and the action row keeps natural widths rather than the kit's block halves, which clipped this app's longer labels |
| MX-VIS-079 | Study session | Exit confirm | `study-session--exit` | **Measured — 28.59% light / 28.96% dark. Blocked on a copy ruling.** The composition is the kit's; the words are not, and here the words decide the layout. §4 writes `Keep studying` / `Save and exit` — which say that leaving *saves* — where the shot writes `Stay` / `Leave`. The app's pair does not fit the kit's side-by-side halves at 390dp, so it wraps and the dialog grows; clipping them to `Keep s…` / `Save a…` was the alternative. Adopt the kit's labels or keep the spec's: an owner call, not a layout one |
