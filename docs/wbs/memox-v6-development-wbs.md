...
| MX-VIS-069 | Today | Loaded | `dashboard--loaded` | **Playwright FAIL — 8.35% light / 10.43% dark** (first measurement 2026-07-26; recorded gap). Every kit section is built and in the kit's order: greeting, Continue-studying, Daily-goal card, stat strip, Recent decks. The residual is four things that cannot be built here — the strip's two unsourced stats (`int-12`), which shorten it by a row and shift everything below up; the app bar's notification bell and avatar (WBS 14.x); and the create FAB's sheet (`int-3`). Residuals by choice: the deck-row mastery bar is 4 logical where the kit draws 6, the stat glyph 22 where the kit draws 24, and the goal caption counts Cards where the shot says minutes — all three kit values are off their own contracts, and this screen's handoff note says tokens only |
...
| MX-VIS-076 | Today | Create sheet | `dashboard--create-sheet` | Blocked — the FAB create sheet on Today is the Create Deck dialog, whose own rows are blocked on `int-3` |
