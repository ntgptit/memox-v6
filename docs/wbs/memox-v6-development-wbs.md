...
| **In progress** | `3.15`, `5.3.2` (child A Done), `int-2` | Phase 0 now closes per vertical slice (owner, 2026-07-20). The **onboarding slice closed 2026-07-21**: 11 `MX-VIS-*` IDs × 2 themes measured, 22/22 PASS, worst 1.93%. `5.3.2` children B and C are pending; `int-2` records Library chrome that landed ahead of its owning `5.4.2`/`10.2` rows. |
| **Done** | `P0.1`, `P0.4`, `P0.6` | **Phase 0 closed by owner decision, 2026-07-26.** `P0.1`: the five rows awaiting a kit decision (`003`, `006`, `007`, `008`, `024`) are resolved as *not measurable — no kit reference*, asserted by widget test as `MX-VIS-039`/`061` already were; the two assertions that did not yet exist were written first. `P0.4`: re-scoped to "measured or explicitly accounted", the shape `parity_census.mjs` already enforced — the literal "ratio for every ID" was unsatisfiable, since some states need unbuilt features and others are forbidden by §6.6. `P0.6`: all three `AC-WBS-P0-06` clauses met. |
| **Open** | All UI rows of `5.6.*`, `5.7.*`, `6.*`–`16.*` | **STOP RULE LIFTED (owner, 2026-07-26)** with the closure of `P0.6`. UI work packages can be promoted to `Ready` again. **The §6.5 PROCESS RULE stays in force:** no screen-changing PR merges without kit-parity evidence ≤3%. That rule is now machine-enforced — CI's `visual_parity` job runs the ratchet and `gate` requires it whenever `scope` reports `visual_changes` — which is precisely why lifting the stop rule is safe: the guarantee it was standing in for is now automatic rather than manual. |
| Blocked | All remaining implementation rows | Preserve dependency order; create/review the item packet immediately before promotion. PROCESS RULE (owner, 2026-07-19): no screen-changing PR merges without kit-parity evidence <3%. `5.4.4` and `5.4.5` **Done 2026-07-25** — the attempt/schedule transaction (schema v2 timestamps, concurrent-writer interleaving, exactly-once across restart) and the progress-test row, whose audit found five of its seven categories already covered by `5.4.2`–`5.4.4` and added the two that were not: the due boundary either side of equality, and the read-side timezone contract. **Correction (2026-07-26): the whole `5.5` Study Mode stream is already built and tested** — six modes, the pure strategy template, the round-order policy, six concrete strategies, the injected `StudyModeFactory` and a contract suite covering every case the `5.5.6` boundary names; 68 tests pass. It is recorded here as Blocked only because no `WBS-5.5.*` item packet exists and §6.2 requires one before promotion. Verified row by row in `int-4`; the Done/packet decision is the owner's. `5.3.2` children B and C are closed except the CF-05 audio control, which the kit itself defers to implementation-time authoring. |
...
- **What the ratio cannot see (2026-07-26).** A pixel ratio is a measure of
  *area*, so on a sparse screen it is nearly blind to structure. Measured, not
  ...
  `tool/parity/geometry_report.mjs` does that comparison for any measured
  state. Its second use corrected a wrong diagnosis: `MX-VIS-057`'s dark gap
  was recorded as shot content the journey could not produce, and was in fact
  `MxBanner` padding 8 logical taller than the kit's.
  ...
  Both were visible to the eye and invisible to the gate.
...
| MX-VIS-003 | First-run language (step 1) | Empty draft | *no kit reference* | **Not measurable — no kit reference; asserted, not photographed** (owner, 2026-07-26). The kit draws no empty-draft frame for step 1. The state's contract is that Continue stays disabled until both selectors are set, pinned by `first_run_language_screen_test.dart` ("continue stays disabled until both selectors are set") |
...
| MX-VIS-006 | Language select sheet | Unfiltered list | *no kit reference* (nearest `languages--list`) | **Not measurable — no kit reference; asserted, not photographed** (owner, 2026-07-26); nearest shot `languages--list` is not authoritative. Pinned by `first_run_language_screen_test.dart` ("the sheet opens on the full supported list"), added 2026-07-26 — the filter test only proved what disappears, never what is there before anyone types |
| MX-VIS-007 | Language select sheet | Search filtered | *no kit reference* | **Not measurable — no kit reference; asserted, not photographed** (owner, 2026-07-26). Pinned by `first_run_language_screen_test.dart` ("the sheet search filters languages") |
| MX-VIS-008 | Language select sheet | No match | *no kit reference* | **Not measurable — no kit reference; asserted, not photographed** (owner, 2026-07-26). Pinned in the same filter test, which ends by asserting "No languages match your search." |
| MX-VIS-009 | First-run deck setup (step 2) | Name filled | `create-deck-firstrun--step2` | **Playwright PASS — 0.80% light / 1.29% dark** (fresh launch → step 1 → step 2 with the deck name filled; capture at `H`, journey continues to Library) |
| MX-VIS-010 | First-run deck setup (step 2) | Optional section expanded | `create-deck-firstrun--step2-optional` | **Playwright PASS — 0.88% light / 1.64% dark** (step 2 → expand OPTIONAL → type the description; capture at `H`, journey continues to Library). Remediated in P0.5: the field rested at two rows because `MxTextField` defaulted multiline `minLines` to 2. It is now `MxTextArea(rows: 1)` — still multi-line, per `edit-deck.md` §Description |
| MX-VIS-011 | First-run deck setup (step 2) | Submitting | `create-deck-firstrun--submitting` | **Playwright PASS — 0.83% light / 0.91% dark** (step 2 → press Create deck against a create command the fixture pins on an unresolved completer; capture at `I`, then §7 asserted: the field and the CTA are both inert) |
...
| MX-VIS-014 | First-run deck setup (step 2) | Name too long | `create-deck-firstrun--name-too-long` | **Playwright PASS — 1.86% light / 1.90% dark** (types the kit's own 89-character name, submits, keeps it, then recovers). Closed the `5.2.1` gap: no deck-name length rule existed. `deckNameMaxLength = 60` is chosen in `deck_name.dart` with its reasoning — no spec, schema or kit fixed a number, so it needs an owner's confirmation |
| MX-VIS-015 | First-run deck setup (step 2) | Resume draft | `create-deck-firstrun--resume-draft` | **Playwright PASS — 0.74% light / 0.75% dark** (types a name, leaves through `Change`, returns, and the draft is there; `Start over` then discards it). Closed the `5.2.3B` gap: the draft persisted but nothing told the user, and there was no way to discard it |
...
| MX-VIS-018 | Library | Empty | `library--empty` | **Playwright PASS — 1.45% light / 1.93% dark** (fresh launch → Today → tap Library tab → empty branch; capture at `Q`, PASS only after Create deck commits and the deck replaces the empty state) |
...
| MX-VIS-020 | Library | Loaded list | `library--loaded` | Blocked — FilterRow, FAB and A-Z sort landed 2026-07-22; the deck-card SRS counters were fixed by `5.4.2` on 2026-07-25 (`int-2` closed) and now render real values. Remaining blocker is the app-bar chrome (search/avatar, 5.x/9.x) |
...
| MX-VIS-024 | Library | Error | *no kit reference* (nearest `library--offline`) | **Not measurable — no kit reference; asserted, not photographed** (owner, 2026-07-26); nearest shot `library--offline` is not authoritative. Pinned by `library_screen_test.dart` ("a failed root read shows the error, not an empty library"), added 2026-07-26. The assertion that matters is the *distinction*: a failed read must not render as "you have no decks", which would invite creating a deck the learner already has. **Follow-up:** the Library error has no Retry, the same dead end `recover-search-failure.md` closed for Search — needs its own row |
...
| MX-VIS-036 | Deck detail — parent branch | Loaded | `subdeck-list--loaded` | **Measured — 12.11% light / 18.15% dark. Permanent recorded gap (owner, 2026-07-26): the JSX is the standard and the shot will not be regenerated.** The screen now renders what the kit `SubdeckList.jsx` renders: `crumbs + FilterRow + list`, with the breadcrumb drawn on root decks, no full-width Study CTA, and create-deck in the shell FAB. **The kit contradicts itself here** — the shot draws a `DECKS · 217 cards · 23 due` section and no filter chips. Owner ruling (2026-07-25): the JSX is the source of truth, so following it moved the ratio slightly the wrong way (12.08 → 12.14 light, 17.66 → 18.20 dark) and that is the accepted cost; the row cannot reach ≤3% against this shot, and the owner has ruled 2026-07-26 that it stays that way rather than spending design time redrawing it — the ratchet in `known-gaps.json` still blocks every other regression. One consequence worth knowing: the scope totals the shot shows are no longer displayed anywhere, because the JSX has no place for them. Still outstanding: `Deck` has no icon field, so every row draws the same glyph — the schema change is owner-approved but **no spec describes deck icons at all** (`docs/business/deck/**` and `docs/database/schema-v1.md` are both silent), so the column alone would render nothing and a picker would be invented product |
| MX-VIS-037 | Deck detail — parent branch | Loading | `subdeck-list--loading` | **Measured — 8.12% light / 9.70% dark. Blocked on the same kit contradiction as `MX-VIS-036`.** Its shot shows a breadcrumb placeholder and a `DECKS` label over deck-row cards — a skeleton of the *loaded* chrome the JSX does not render either. Under the owner's JSX ruling this row waits on the same regenerated shots. The number moved slightly against it when the loaded branch gained its FAB, which the loading capture also shows; not chased, because the composition it is being compared against is the thing in question |
| MX-VIS-038 | Deck detail — parent branch | Error | `subdeck-list--error` | **Playwright PASS — 2.31% light / 2.65% dark** (Library → open the parent, child read broken at the use case). The two attempts on 2026-07-25 overrode `deckChildrenProvider`, the derived *presentation* provider, and the parity build stayed on its loading skeleton; the cause was recorded as "the parity build itself". It was narrower: `MX-VIS-037`/`043` already prove an override of `openDeckUseCaseProvider` reaches the web build, so the use case is a seam that works and the derived provider is one that does not. Breaking `childrenOf` there was the whole fix — no harness change. The spec asserts the failure copy before capturing, so a skeleton can no longer be mistaken for the state the way the ratio alone allowed |
| MX-VIS-039 | Deck detail — parent branch | Not found | `subdeck-list--not-found` | **Structurally unreachable under §6.6 — asserted, not photographed.** Composition built: the kit's `folder_off` warning tile, `This deck no longer exists`, a single Back to Library action, and an app bar stripped of search and overflow, the kit's reason being that nothing left to search or configure survives the deck. Reaching it needs a deck to vanish while its screen is open, and no compliant journey does that: deleting the deck you are viewing pops the dialog and calls `goLibrary()` by design (`delete_deck_dialog.dart`), so the app deliberately leaves before the state could render. What remains — restoring a stale route, browser history, or an import/sync removing it — is either route re-entry (a deep link by another name, banned by VP-04) or unbuilt (13.x). Same class as `MX-VIS-013`/`027`. **Substitute assurance:** three widget tests pin the composition, the stripped app bar, and — added 2026-07-26 — that the single exit actually navigates. That last one is the point: the state is unmeasurable, so if `Back to Library` stopped working no parity run would ever notice, and a dead end whose only way out is broken is the worst version of this screen. **Unblocked by** any flow that removes a deck out from under an open screen — import/sync (13.x) or multi-device ||
...
| MX-VIS-042 | Deck detail — leaf branch | Loaded | `flashcard-list--loaded` | Blocked — filter chips (10.2), SRS counters/badges (5.4.2), breadcrumb (6.2). **Add-card affordance closed 2026-07-27** (`int-52`): the full-width button the kit does not draw is gone, and the FAB adds a card the way `FlashcardList.jsx` does. It reached this state by the branch it belongs to — the FAB used to create a *deck* here, the one action `organise-deck.md` §2 blocks on a Leaf, which is why the button existed at all. The kit opens an `AddCardSheet` (Add card / Import cards) rather than the editor directly; a two-row sheet whose second row imports nothing is the same dishonesty `MX-VIS-076` refuses, so the FAB goes straight to the editor until import exists. Not measurable yet regardless: the three blockers above are unbuilt composition, so a capture would photograph their absence |
| MX-VIS-043 | Deck detail — leaf branch | Loading | `flashcard-list--loading` | **Playwright PASS — 1.21% light / 1.05% dark** (fresh launch → Library tab → open the childless deck; the fixture pins only the card stream on a never-emitting controller, so the branch resolves to Leaf and the capture is a still frame; the journey then returns to Library through the app bar, because a load that never resolves must not be a trap). Closed from 5.32%/11.49% by two measured causes: the placeholder opened without the `space-4` the loaded branches use, so it sat 16px high and the screen jumped when data arrived; and its first line was 14px — the kit's *deck*-row height — where the card row wants 16, a 2px error that compounded down five rows |
| MX-VIS-044 | Deck detail — leaf branch | Error | `flashcard-list--error` | **Playwright PASS — 2.29% light / 2.61% dark** (Library → open the leaf, card read broken at the use case). Unblocked by the same seam correction as `MX-VIS-038`, which is where the reasoning is written down |
...
| MX-VIS-049 | Card Editor | Create | `flashcard-editor--create` | **Playwright PASS — 1.25% light / 1.68% dark** (fresh app launch → first-use Create Deck A→K → user opens Empty Deck → Create Flashcard A→J; capture at C, PASS only after Save returns to Leaf list with the committed Card visible) |
...
| MX-VIS-054 | Study Result | Standard | `study-result--standard` | **Playwright PASS — 1.78% light / 2.40% dark** |
| MX-VIS-062 | Match | Playing | `match-mode--playing` | **Playwright PASS — 0.45% light / 0.48% dark** |
| MX-VIS-063 | Match | Selected | `match-mode--selected` | **Playwright PASS — 0.55% light / 1.08% dark** (resume into Match, tap the second left-hand tile) |
| MX-VIS-064 | Match | Correct | `match-mode--correct` | **Playwright PASS — 0.74% light / 0.79% dark**, from 16.00 / 11.57. Two fixes: the kit's tone map (`success-soft`, and a matched tile leaves the board), then the tile height — `min-height: size-xl + space-5` = 116 logical against this build's 72. The height was the dominant term |
| MX-VIS-065 | Match | Wrong | `match-mode--wrong` | **Playwright PASS — 1.18% light / 1.30% dark**, from 12.07 / 4.03 |
| MX-VIS-066 | Match | Almost | `match-mode--almost` | **Playwright PASS — 0.87% light / 1.31% dark** (resume into Match → clear three pairs → cancel a selection). This row previously read "the near-miss cue (`SM-MATCH-*` classification)", which is wrong and would have sent the next person building a duplicate-normalized-meaning fixture: `MatchMode.jsx` tones three tiles a side `matched` and shows 12/20, so `almost` means the board is almost *finished*, not that a pairing was almost right. First measured 13.37% because the last match keeps flashing until the next interaction and every interaction left a selection behind — the kit's resting frame was unreachable until a selected tile could be cancelled |
| MX-VIS-067 | Match | Round complete | `match-mode--complete` | **Playwright PASS — 0.53% light / 0.53% dark** (resume into Match → clear all five pairs). First measured 5.70/7.08: `Next round` was a sticky `bottomBar`, where the kit hangs it off the `EmptyState` as that state's own action — ~470 logical lower than the copy it belongs to — and the celebration tile was toned primary where the kit tones it success |
| MX-VIS-068 | Today | Loading | `dashboard--loading` | Composition built 2026-07-26 — greeting + the kit skeleton (section label, primary action, goal card, second label, two deck rows), replacing a bare loading label. Not yet measured: the surrounding sections it stands in for do not exist yet, so a ratio would report the gap between this state and an unbuilt screen rather than anything about the skeleton |
| MX-VIS-069 | Today | Loaded | `dashboard--loaded` | **Playwright FAIL — 7.67% light / 9.45% dark** (first measured 2026-07-26 at 8.35 / 10.43; re-measured 2026-07-27 after the create FAB landed. The dark figure is stable across two runs). Every kit section is built and in the kit's order: greeting, Continue-studying, Daily-goal card, stat strip, Recent decks, and the create FAB. The residual is three things that cannot be built here — the strip's two unsourced stats (`int-12`), which shorten it by a row and shift everything below up, and the app bar's notification bell and avatar (WBS 14.x). Residuals by choice: the deck-row mastery bar is 4 logical where the kit draws 6, the stat glyph 22 where the kit draws 24, and the goal caption counts Cards where the shot says minutes — all three kit values are off their own contracts, and this screen's handoff note says tokens only |
| MX-VIS-070 | Today | Not studied | `dashboard--not-studied` | Composition partial — the nudge note, stat strip and Recent decks are in place; the strip's two unsourced stats are `int-12` |
| MX-VIS-071 | Today | Goal met | `dashboard--goal-met` | Composition partial — the card's met state, the celebration note, the stat strip and Recent decks are in place; the strip's two unsourced stats are `int-12`. The note does not carry the kit's "Streak +1": a streak day is earned by the day's first qualified card, not by the target (`record-streak-day.md` §6) |
| MX-VIS-072 | Today | Streak reset | `dashboard--streak-reset` | Composition partial — the streak-reset note (gated on streak history, so a first-time learner is never told a run they never had has ended), the stat strip and Recent decks are in place; the strip's two unsourced stats are `int-12` |
| MX-VIS-073 | Today | Caught up | `dashboard--caught-up` | Composition partial — the kit's inline caught-up section (message + `Explore decks`), the stat strip and Recent decks are in place, each deck row marked with a check rather than a zero badge; the strip's two unsourced stats are `int-12` |
| MX-VIS-074 | Today | Empty | `dashboard--empty` | Composition partial — the empty state exists; the kit also draws an onboarding hero and a "How MemoX works" section |
| MX-VIS-075 | Today | Empty after onboarding skip | `dashboard--empty-after-onboarding-skip` | Blocked — depends on the first-run "Not now" branch (`MX-VIS-002`), itself blocked because its shot is this Today state rather than a landing one |
| MX-VIS-076 | Today | Create sheet | `dashboard--create-sheet` | Composition partial — the FAB and its sheet landed 2026-07-26 (`manage-today-create-actions.md`): Add card resolves an eligible target before handing off, Create deck opens the existing dialog and refreshes Today once. The kit's third row, `Import cards`, is **not offered**: nothing in this build imports anything — no route, flow or use case — and a menu row that opens nothing is worse than a menu that is honest. Measuring is blocked on that row and on the kit's own `add-card-target` picker, which the multi-deck branch would need |
| MX-VIS-077 | Study Result | Finalize error | `study-result--finalize-error` | **Playwright PASS — 1.61% light / 2.50% dark** (deep link to `/study` with the finalize commit failing). The state was built 2026-07-27 (`int-49`) and had no row; the kit's own `StudyResult.jsx` branch is what it now follows — an error-toned `cloud_off` EmptyState over `Retry` + `Not now`. Two things the measurement decided rather than the eye: the action column is `--memox-size-3xl` (the `standard` width, not `wide`, which painted past the kit's buttons on both sides), and the bottom nav reservation stays — removing it took the diff from 3.47% to 9.36% |
| MX-VIS-078 | Study session | Answer save error | `study-session--answer-save-error` | **Measured — 24.88% light / 25.29% dark. Same kit self-contradiction as `MX-VIS-036`/`037`, one screen over.** `ReviewMode.jsx` gives a study screen a nested bar (back arrow, mode title, `format_size`, `more_vert`) with the progress header as the body's first child — what this build renders; the `study-session--*` shots draw an X, the progress inline in the bar, one overflow. The app follows the JSX per the 2026-07-25 ruling. Measured rather than assumed: seeding the shot's own scenario (25 cards, the 21st, prompting `school`) moved it 24.50 → 24.88, so the counter and prompt were never the residual. The dialog itself is the kit's composite (`int-54`) |
| MX-VIS-079 | Study session | Exit confirm | `study-session--exit` | **Measured — 28.59% light / 28.96% dark. Blocked on the same contradiction plus a copy ruling.** §4 writes `Keep studying` / `Save and exit` — which say that leaving *saves* — where the shot writes `Stay` / `Leave`. The app's pair does not fit the kit's side-by-side halves at 390dp, so it wraps and the dialog grows; clipping them to `Keep s…` / `Save a…` was the alternative (`int-55`). Adopt the kit's labels or keep the spec's: an owner call |
| MX-VIS-080 | Study Result | Goal met | `study-result--goal-met` | **Playwright PASS — 2.23% light / 2.18% dark** (deep link to `/study` with a committed summary whose contribution crossed the target). The state was built this round (`int-72`); the screen previously had one hero for every outcome. Two deliberate residuals, both already ruled on: the goal row reads `22/20 cards` where the shot reads `22/20 min` (`int-8` — `metrics-v1` fixes the v1 unit as Cards and `StreakGoalCard.d.ts` records its own numbers as hardcoded samples), and the hero body reads `22 of 20 completed today` where the shot reads `Streak +1 → 13 days in a row.` — nothing in this build knows whether *this* session is what extended the streak, and `complete-daily-goal.md` §4 names the line that is both true and specified. The streak caption drops the shot's ` · +1 today` for the same reason. **What measuring found:** the first run came in at 3.07% light, and the cause was not copy — `MxBadge` centres its content, so under the loose constraints an `Align` hands down it stretched to the full card width instead of wrapping its label. A min-size `Row` fixed it and took light from 3.07 to 2.23; the constraint is now documented on the widget |
| MX-VIS-081 | Study Result | Finalizing | `study-result--finalizing` | **Playwright PASS — 1.00% light / 1.64% dark** (deep link to `/study` with the finalize commit pinned in flight). Built this round (`int-74`): the state was one centred sentence where the kit spends the whole screen — hero, three stat placeholders and the streak card's — so the screen went from a line of text to a full page the moment the commit landed. The title also adopts the kit's `Saving your results…` in place of `Finalizing…`, which no flow specified |
| MX-VIS-082 | Study Result | Retry finalize | `study-result--retry-finalize` | **Playwright PASS — 0.82% light / 1.44% dark** (the same view after a failed finalize was retried). Built this round (`int-75`): `finalize-study-session.md` §9 lists `retry` as its own state beside `finalizing` and the kit reframes the view for it — glyph, title and body all change. The app had no such state, so a learner whose save had just gone wrong pressed Retry and saw the first-attempt copy |
| MX-VIS-083 | Search | Empty / recent | `search--empty-recent` | Pending measurement — registered 2026-07-28 (`int-105`). The kit ships this state as a light+dark pair and the app has the surface, so `deck/README.md` §13 — parity for every canonical state × theme *that has a UI reference* — already puts it in scope. It had no row, so "no evidence" was indistinguishable from "out of scope", and the §6.5 merge rule had nothing to protect these screens with |
| MX-VIS-084 | Search | Results | `search--results` | **Measured 2026-07-28 — 4.37% light / 6.57% dark** (`int-106`), over the kit's own scenario: three cards across two decks, one hidden, query `하`. The row was rebuilt to `StatusCardRow` this round — term over meaning over deck line, no leading icon, hidden as the crossed-eye glyph with the row dimmed. The first number on this row (1.95/3.20) was measured over a sparse fixture and was flattered by it; matching the shot's content raised the ratio and made the composition gaps legible. Four residuals, all recorded in `known-gaps.json`: the search dock the screen does not use, the SRS badge the index is forbidden to feed, two unbuilt filter dimensions, and the hidden-row dimming a guard keeps out of the feature layer — worth a measured 0.44/0.83 |
| MX-VIS-085 | Search | Filtered | `search--filtered` | Pending measurement — registered 2026-07-28 (`int-105`). The kit ships this state as a light+dark pair and the app has the surface, so `deck/README.md` §13 — parity for every canonical state × theme *that has a UI reference* — already puts it in scope. It had no row, so "no evidence" was indistinguishable from "out of scope", and the §6.5 merge rule had nothing to protect these screens with |
| MX-VIS-086 | Search | No results | `search--no-results` | Pending measurement — registered 2026-07-28 (`int-105`). The kit ships this state as a light+dark pair and the app has the surface, so `deck/README.md` §13 — parity for every canonical state × theme *that has a UI reference* — already puts it in scope. It had no row, so "no evidence" was indistinguishable from "out of scope", and the §6.5 merge rule had nothing to protect these screens with |
| MX-VIS-087 | Search | Loading | `search--loading` | Pending measurement — registered 2026-07-28 (`int-105`). The kit ships this state as a light+dark pair and the app has the surface, so `deck/README.md` §13 — parity for every canonical state × theme *that has a UI reference* — already puts it in scope. It had no row, so "no evidence" was indistinguishable from "out of scope", and the §6.5 merge rule had nothing to protect these screens with |
| MX-VIS-088 | Deck Settings | Reset confirm | `deck-settings--reset-confirm` | Pending measurement — registered 2026-07-28 (`int-105`). The kit ships this state as a light+dark pair and the app has the surface, so `deck/README.md` §13 — parity for every canonical state × theme *that has a UI reference* — already puts it in scope. It had no row, so "no evidence" was indistinguishable from "out of scope", and the §6.5 merge rule had nothing to protect these screens with |
| MX-VIS-089 | Deck Settings | Delete confirm | `deck-settings--delete-confirm` | Pending measurement — registered 2026-07-28 (`int-105`). The kit ships this state as a light+dark pair and the app has the surface, so `deck/README.md` §13 — parity for every canonical state × theme *that has a UI reference* — already puts it in scope. It had no row, so "no evidence" was indistinguishable from "out of scope", and the §6.5 merge rule had nothing to protect these screens with |
| MX-VIS-090 | Mode Picker | Default | `mode-picker--default` | **Measured 2026-07-28 — 13.33% light / 18.39% dark** (`int-110`). Registered unmeasured last round and measured this one, which is the order `int-105` asks for. The composition is the kit's and in its order; the residual is a cumulative vertical drift whose cause is narrowed by measurement to row height rather than section gaps — switching to the kit's single list rhythm moved light 1.35 and dark 0.01. Recorded in `known-gaps.json` with the next step: compare the row metrics against the kit's card |
| MX-VIS-091 | Mode Picker | Scope dropdown | `mode-picker--scope-dropdown` | Pending measurement — registered 2026-07-28 (`int-109`). The screen was built this round from `ModePicker.jsx`; the shot exists, so `deck/README.md` §13 puts it in scope. Registering it without a number is bookkeeping, which `int-105` says out loud — the measurement is the next round's first job, and the two states below it are further out: the scope sheet is unbuilt and the not-enough callout needs the Guess-pool read the picker does not do yet |
| MX-VIS-092 | Mode Picker | Not enough | `mode-picker--not-enough` | Pending measurement — registered 2026-07-28 (`int-109`). The screen was built this round from `ModePicker.jsx`; the shot exists, so `deck/README.md` §13 puts it in scope. Registering it without a number is bookkeeping, which `int-105` says out loud — the measurement is the next round's first job, and the two states below it are further out: the scope sheet is unbuilt and the not-enough callout needs the Guess-pool read the picker does not do yet |
| MX-VIS-055 | Card Editor | Duplicate | `flashcard-editor--duplicate` | **Playwright PASS — 2.07% light / 2.31% dark** (fresh launch → first-use Create Deck → open deck → save first Card → **Add card again** with the same term → duplicate review). No fixture override: the duplicate is produced by the real create path, because detection reads the normalized content that path writes. Reaching it at all required the disabled leaf-deck `Add card` fix (PR #126) |
| MX-VIS-056 | Card Editor | Submitting | `flashcard-editor--submitting` | **Playwright PASS — 1.09% light / 1.34% dark** (create journey → Save with the card write pinned on a completer nothing resolves; the fields freeze and Save reads "Saving…") |
| MX-VIS-057 | Card Editor | Submit error | `flashcard-editor--submit-error` | **Playwright PASS — 0.87% light / 2.76% dark**, from 1.72 / 4.32. **The recorded reason for the dark gap was wrong.** It read "the shot embeds an additional-translation row and tag content the journey does not create"; the actual cause was `MxBanner`'s inline layout padding every side with `space-4` and using the card radius, where the kit's `ActionCallout` is `space-3` vertical / `space-4` horizontal at `radius-control`. That made every inline banner 8 logical taller than the kit's and pushed each band below it out of alignment. Found by row-band comparison, not by looking harder at the content |
| MX-VIS-058 | Card Editor | Validation | `flashcard-editor--validation` | **Playwright PASS — 2.20% light / 2.51% dark** (create journey → type into each required field, then clear it). A required field reports its error once *touched*, never on arrival, so typing-then-clearing is the only production path to the kit's both-errors frame — the state is not reachable by loading the form. Capture needed focus parked on the inert deck-context pill first: `fillField` blurs with Tab, which in this form lands on the next text field, so a blinking caret made three consecutive settles disagree. Parking the caret is not the same as dropping focus: once the Meaning label gained its `+` (WBS 6.4 create entry point), Tab reached that button and it drew a focus ring, pushing dark to 3.01%. The spec now blurs the active element outright — the ring is correct behaviour captured at the wrong moment, the pointer-parking lesson one input device over |
| MX-VIS-059 | Card Editor | Edit | `flashcard-editor--edit` | **Playwright PASS — 0.97% light / 1.79% dark** (create the kit's card, reopen it, amend the meaning to the shot's "Hello (formal)"). Came down from 7.00 / 7.54 in three steps: the kit's progressive disclosure (translation slot behind the Meaning `+`), the kit's `TagsField` (one bordered row — `sell` glyph, plain chips, inline caret), and finally the journey capturing a **dirty** edit. Only the last was decisive: the kit's JSX sets `isDirty = view === 'edit'` and draws Save at full strength, while this app disables Save until content diverges (`edit-flashcard.md` §6), so a clean-edit capture was comparing a dimmed control against an enabled one — **6 of the 9.5 percentage points sat in the footer alone** |
| MX-VIS-060 | Card Editor | Additional translation | `flashcard-editor--additional-translation` | **Playwright PASS — 1.49% light / 2.61% dark** (create the kit's card, reopen it, disclose the slot from the Meaning `+`, add "Xin chào", amend the meaning). Banding by contribution *first* found the same footer signature as `MX-VIS-059` — 6.03 of 10.32pp in three bands around the sticky Save — so the one change that mattered was dirtying the form: a translation persists on its own, leaving the card content clean and Save dimmed against the kit's enabled one. 6.82 → 1.40 in a single step |
| MX-VIS-061 | Card Editor | Keyboard open | `flashcard-editor--keyboard-open` | **Structurally unreachable — not measurable, by construction.** The kit renders this state with a *simulated software keyboard* drawn into the mock (`keyboard-open renders the populated edit form with a simulated software keyboard raised`); a Flutter Web capture in headless Chromium has no OS keyboard and the app never draws one, so no journey can produce those pixels. Same class as `MX-VIS-013`/`027`. **The contract the state exists for is asserted instead of photographed:** KIT-25-04/35-01 requires the sticky Save bar to sit above the raised keyboard and never be covered, and the widget test *"the Save bar stays above a raised keyboard"* raises a 600px inset and checks the button moved up into the remaining area. Verified by mutation — setting `resizeToAvoidBottomInset: false` fails it |
