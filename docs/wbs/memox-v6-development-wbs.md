...
| Blocked | All remaining implementation rows | Preserve dependency order; create/review the item packet immediately before promotion. PROCESS RULE (owner, 2026-07-19): no screen-changing PR merges without kit-parity evidence <3%. Next: `5.4.4` (attempt/schedule transaction) — the only writer of what `5.4.3` computes; it owns `SRS8-011`/`-012` and the `srsActivatedAt`/`lastReviewedAt` schema addition. Then `5.4.5`, then `5.3.2` children B/C. |
