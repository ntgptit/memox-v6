...
| MX-VIS-036 | Deck detail — parent branch | Loaded | `subdeck-list--loaded` | **Measured — 12.08% light / 17.66% dark, down from 20.26 / 28.84. Blocked on the kit, not on the app.** Three composition gaps closed on 2026-07-25 (owner-approved): the breadcrumb now draws on a root deck, the full-width Study CTA is gone, and create-deck moved into the shell FAB. All three are what the kit `SubdeckList.jsx` renders, so they stand regardless of the conflict below. **The kit contradicts itself for this state:** the JSX `loaded` branch renders `crumbs + FilterRow + list`, while the shot draws a `DECKS · 217 cards · 23 due` section and no filter chips. Owner ruling (2026-07-25): **the JSX is the source of truth**, so the screen will grow the FilterRow and this row cannot reach ≤3% until the Design Kit owner regenerates the shot. Also outstanding: `Deck` has no icon field, so every row draws the same glyph where the kit gives each deck its own (owner-approved schema change, not yet built) |
