...
| MX-VIS-078 | Study session | Answer save error | `study-session--answer-save-error` | **Measured — 18.82% light / 18.72% dark** (was 21.49 / 21.42). The dialog now follows the kit's icon `Dialog`: tone tile on top, copy centred under it, `Back` and `Retry` sharing one row (`int-54`, a shared-composite fix that reaches every confirm in the app). The residual is not the dialog. The kit stages this shot over Stage 5 - Fill at 21/25 on the card 'school'; the fixture is a five-card session on its first card, and the study shell's app bar is the divergence `int-5` owns. Closing it means seeding the shot's own session and finishing `int-5` |
