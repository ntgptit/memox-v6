...
| MX-VIS-043 | Deck detail — leaf branch | Loading | `flashcard-list--loading` | **Playwright PASS — 1.19% light / 1.03% dark** (fresh launch → Library tab → open the childless deck; the fixture pins only the card stream on a never-emitting controller, so the branch resolves to Leaf and the capture is a still frame; the journey then returns to Library through the app bar, because a load that never resolves must not be a trap). Closed from 5.32%/11.49% by two measured causes: the placeholder opened without the `space-4` the loaded branches use, so it sat 16px high and the screen jumped when data arrived; and its first line was 14px — the kit's *deck*-row height — where the card row wants 16, a 2px error that compounded down five rows |
