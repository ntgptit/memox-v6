...
| MX-VIS-069 | Today | Loaded | `dashboard--loaded` | **Playwright FAIL — 7.67% light / 10.43% dark** (first measured 2026-07-26 at 8.35 / 10.43; the create FAB closed part of the light gap). Every kit section is built and in the kit's order: greeting, Continue-studying, Daily-goal card, stat strip, Recent decks, and the create FAB. The residual is three things that cannot be built here — the strip's two unsourced stats (`int-12`), which shorten it by a row and shift everything below up, and the app bar's notification bell and avatar (WBS 14.x). Residuals by choice: the deck-row mastery bar is 4 logical where the kit draws 6, the stat glyph 22 where the kit draws 24, and the goal caption counts Cards where the shot says minutes — all three kit values are off their own contracts, and this screen's handoff note says tokens only |
