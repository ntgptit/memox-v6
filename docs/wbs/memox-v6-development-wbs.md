...
| MX-VIS-059 | Card Editor | Edit | `flashcard-editor--edit` | **Measured 7.00% light / 7.54% dark — blocked on `5.3.2` child C**, not tunable. The journey builds the kit's own card (term 안녕하세요, meaning "Hello (formal)", tags #TOPIK_I / #인사) and reopens it for edit, so content matches; the residual is three surfaces child C owns and this build does not have: the kit's Term field carries a trailing **audio** control, its Meaning label carries a **`+`** to add a translation, and its tags render as **chips** — where this build has no audio affordance, a separate "ADDITIONAL TRANSLATIONS" section, and a free-text tags field. Prefill itself is correct: the capture shows both stored values painted, and the controllers are asserted by the widget test |
