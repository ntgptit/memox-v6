...
| MX-VIS-069 | Today | Loaded | `dashboard--loaded` | Composition partial — greeting, Continue-studying, the Daily-goal card and the Recent-decks list are all in place, in the kit's order. Outstanding: the four-stat Today strip, and the section caption's deck count (`dueCount` is card-scoped, with no per-deck breakdown composed). Residual: the kit's deck-row mastery bar is 6 logical tall, off its own spacing scale, so the standard 4px bar is used (WBS 5.7.2) |
| MX-VIS-070 | Today | Not studied | `dashboard--not-studied` | Composition partial — the nudge note and the Recent-decks list landed 2026-07-26; the four-stat Today strip is outstanding |
| MX-VIS-071 | Today | Goal met | `dashboard--goal-met` | Composition partial — the card's met state, the celebration note and the Recent-decks list are in place; the four-stat Today strip is outstanding. The note does not carry the kit's "Streak +1": a streak day is earned by the day's first qualified card, not by the target (`record-streak-day.md` §6) |
| MX-VIS-072 | Today | Streak reset | `dashboard--streak-reset` | Composition partial — the streak-reset note (gated on streak history, so a first-time learner is never told a run they never had has ended) and the Recent-decks list are in place; the four-stat Today strip is outstanding |
| MX-VIS-073 | Today | Caught up | `dashboard--caught-up` | Composition partial — the kit's inline caught-up section (message + `Explore decks`) and the Recent-decks list are in place, each deck row marked with a check rather than a zero badge; the four-stat Today strip is outstanding |
