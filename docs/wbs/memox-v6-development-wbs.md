...
| MX-VIS-069 | Today | Loaded | `dashboard--loaded` | Composition partial — greeting, Continue-studying and the Daily-goal card are in place, in the kit's order as of 2026-07-26 (the card had landed above the primary section, against spec §3 "due/resume content precedes goal/streak content"). Outstanding: the Today inline strip, the Recent-decks list, and the section caption's deck count — `dueCount` is card-scoped, with no per-deck breakdown composed (WBS 5.7.2) |
| MX-VIS-070 | Today | Not studied | `dashboard--not-studied` | Composition partial — the nudge note landed 2026-07-26 (shown when a run is live and today is not a qualified day yet); the Today inline strip and Recent-decks list are outstanding |
| MX-VIS-071 | Today | Goal met | `dashboard--goal-met` | Composition partial — the card's met state and the celebration note both landed 2026-07-26; the Today inline strip and Recent-decks list are outstanding. The note does not carry the kit's "Streak +1": a streak day is earned by the day's first qualified card, not by the target (`record-streak-day.md` §6) |
| MX-VIS-072 | Today | Streak reset | `dashboard--streak-reset` | Composition partial — the streak-reset note landed 2026-07-26, gated on streak history so a first-time learner is never told a run they never had has ended; the Today inline strip and Recent-decks list are outstanding |
| MX-VIS-073 | Today | Caught up | `dashboard--caught-up` | Composition partial — rebuilt 2026-07-26 as the kit's inline section (message + `Explore decks`) instead of a centred empty state that offered no control at all; the greeting is in place, the strip and deck rows are not |
