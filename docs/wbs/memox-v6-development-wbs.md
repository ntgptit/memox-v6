...
| MX-VIS-069 | Today | Loaded | `dashboard--loaded` | Composition partial — greeting done; the Today inline strip, Daily-goal card, Continue-studying section and Recent-decks list are outstanding (WBS 5.7.2). `TodayProjection.dailyProgress` already carries the goal and streak the card needs |
| MX-VIS-070 | Today | Not studied | `dashboard--not-studied` | Composition partial — same sections as `MX-VIS-069` |
| MX-VIS-071 | Today | Goal met | `dashboard--goal-met` | Composition partial — needs the Daily-goal card in its met state; the data exists (`DailyProgressStatus.isMet`) |
| MX-VIS-072 | Today | Streak reset | `dashboard--streak-reset` | Composition partial — needs the Daily-goal card; the streak projection already reports a broken run as `current = 0` |
