...
| MX-VIS-069 | Today | Loaded | `dashboard--loaded` | Composition partial — every kit section is built: greeting, Continue-studying, Daily-goal card, stat strip and Recent decks, in the kit's order. Outstanding by source, not by build: the strip carries 2 of the kit's 4 stats (`int-12`), and the section caption has no deck count because `dueCount` is card-scoped. Residuals: the deck-row mastery bar is 4 logical where the kit draws 6, and the stat glyph is 22 where the kit draws 24 — both kit values are off their own scales, and the screen's handoff note says tokens only (WBS 5.7.2) |
