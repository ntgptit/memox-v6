...
| MX-VIS-042 | Deck detail — leaf branch | Loaded | `flashcard-list--loaded` | Blocked — filter chips (10.2), SRS counters/badges (5.4.2), breadcrumb (6.2). Also carries a full-width **Add card** button the kit does not draw: `FlashcardList.jsx` gives a populated leaf an `MxFab` opening an `AddCardSheet` (Add card / Import cards). The button was found shipped **disabled** on 2026-07-25 — a deck could take its first card and no others — and was enabled as the minimal fix; swapping it for the kit's FAB + sheet is part of closing this row |
