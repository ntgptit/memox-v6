...
| MX-VIS-036 | Deck detail — parent branch | Loaded | `subdeck-list--loaded` | Blocked, but two of its four gaps are closed and it is still **unmeasured** (no fixture or spec exists yet, so there is no ratio to quote). The breadcrumb shipped with `6.2`; the branch now renders the kit's `DECKS` section label over the same `MxDeckCard` rows the Library root uses, carrying the `5.4.2` due/new counters through a new `watchChildDeckSummaries` read model. Remaining: the per-row play affordance and the app-bar search glyph (`5.x`). **The play affordance was attempted on 2026-07-25 and reverted, and the reason is worth knowing before the next attempt:** putting an interactive control inside the tappable deck card makes Flutter merge the row's children into the card's own accessible name, so the deck title stops being an addressable text node and only survives as part of an `aria-label`. Six specs locate decks with `getByText(deckName)` and all of them broke; parity went 34/36 → 22/36 and recovered exactly on revert. The affordance is correct per the kit — the kit draws the bolt on Library rows too — but it needs the deck card to expose a deliberate `semanticLabel` around a nested action, and the specs moved onto that label, which is its own piece of work rather than a tail-end addition |
