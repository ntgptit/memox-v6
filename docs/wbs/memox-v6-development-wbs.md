...
| MX-VIS-059 | Card Editor | Edit | `flashcard-editor--edit` | **Measured 6.53% light / 7.11% dark — blocked on `5.3.2` child C.** Was 7.00 / 7.54 before the kit's progressive disclosure landed (the translation slot now sits behind a `+` on the Meaning label, so the resting form is Term → Meaning → Tags as the kit draws it). The journey builds the kit's own card, so content matches and the residual is structural. Dominant remainder is the **tags surface**: this build renders an uppercase `TAGS` overline, chips carrying a remove `×`, and a separate *Add tag* field; the kit's `TagsField` is one bordered row — `sell` icon + plain chips — under a sm/bold `Tags` label. Then the Term field's **audio** control (CF-05), which does not exist here at all |
