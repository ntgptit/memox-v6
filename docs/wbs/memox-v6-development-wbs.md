...
| Blocked | All remaining implementation rows | Preserve dependency order; create/review the item packet immediately before promotion. Next promotion candidate once packeted: `4.4` children B/C (progress, session DAOs), with `4.5` mappers eligible in parallel; the 3.12 minimal gate follows 4.10. |
