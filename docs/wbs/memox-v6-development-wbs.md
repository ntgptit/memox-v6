...
| MX-VIS-036 | Deck detail — parent branch | Loaded | `subdeck-list--loaded` | **Measured — 12.11% light / 18.15% dark. Blocked on the kit, and deliberately not optimised against the shot.** The screen now renders what the kit `SubdeckList.jsx` renders: `crumbs + FilterRow + list`, with the breadcrumb drawn on root decks, no full-width Study CTA, and create-deck in the shell FAB. **The kit contradicts itself here** — the shot draws a `DECKS · 217 cards · 23 due` section and no filter chips. Owner ruling (2026-07-25): the JSX is the source of truth, so following it moved the ratio slightly the wrong way (12.08 → 12.14 light, 17.66 → 18.20 dark) and that is the accepted cost; the row cannot reach ≤3% until the Design Kit owner regenerates the shot. One consequence worth knowing: the scope totals the shot shows are no longer displayed anywhere, because the JSX has no place for them. Still outstanding: `Deck` has no icon field, so every row draws the same glyph — the schema change is owner-approved but **no spec describes deck icons at all** (`docs/business/deck/**` and `docs/database/schema-v1.md` are both silent), so the column alone would render nothing and a picker would be invented product |
