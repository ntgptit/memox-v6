...
Phạm vi: ứng dụng Flutter local-first cho Android, iOS, web và desktop  
...
| 3. Why can the design kit not be copied literally? | Its documented target is React Native, phone portrait and 390×780; MemoX v6 is Flutter and multi-platform. | Exact phone parity and adaptive large-screen behavior require two complementary contracts. | Keep 390×780 light/dark as the canonical visual baseline and add a centralized Flutter adaptive profile without changing frozen token/component/node names. |
...
- Business catalog: 126 Markdown files across 11 owning aggregates and 6 supporting capabilities.
...
- Current guard: 246 rules; baseline run passes with 0 errors but 172 warnings, largely because expected architecture paths do not exist yet. `intl: any` is also non-reproducible debt.
- Missing engineering artifacts: architecture ADRs, navigation contract, decision tables, WBS traceability log, database/schema docs, migration fixtures, consolidated verifier and integration-test harness.

### 3.2 Các gate phải dừng để quyết định

No feature implementation starts until DG-01 through DG-04 have an accepted ADR or an explicit product decision.

| ID | Conflict / missing decision | Source A | Source B | Recommended resolution | Required output |
| --- | --- | --- | --- | --- | --- |
| DG-01 | Can a Deck contain direct cards and child Decks simultaneously? | `docs/business/deck/README.md`: mixed content is invalid; Empty becomes Leaf or Parent. | Design `SCOPE.md`: a Deck holds nested decks “and/or cards”. | Business wins: prohibit mixed content; update kit prose/fixtures while preserving frozen identifiers. | `ADR-001-deck-content-model.md`, corrected design scope/specs and guard tests. |
| DG-02 | What is the production platform/layout contract? | Repo/product: Flutter multi-platform. | Kit: React Native, phone portrait, 390×780; tablet/landscape unsupported. | Flutter is production; 390×780 remains the parity baseline; add adaptive Flutter rules for other windows. | `ADR-002-flutter-platform-and-adaptive-scope.md` plus supported window/input matrix. |
| DG-03 | SRS policy v1 | Đã chốt Leitner 8 Box trong `learning-progress/srs-8-box-policy.md`. | Box 0 là new; Box 1..7 dùng interval 1/3/7/14/30/60/120 ngày; Box 8 mastered. | Correct +1 box, wrong -1 box; sticky-wrong terminal aggregation; engine thuần với injected clock. | Policy doc và decision table `SRS8-001..016`. |
| DG-04 | How will guard and architecture be reconciled? | Clean Architecture places SRS calculation in domain policy; business names Deck. | Guard pins `intervalForBox`/`boxAfterFinalization` to data repo files and contains old `folders` target paths. | Fix the guard upstream; do not name app code `Folder` or hide domain logic to evade regex. | Separate guard-repo change, new pinned submodule commit, zero stale-path warnings for active code. |
| DG-05 | Which locales are release-supported? | Flutter repo already has English and Vietnamese ARB files. | Kit declares English/LTR placeholders and lists wider localization as planned. | Support English and Vietnamese in v1; test CJK card content; defer RTL only through an explicit release exception. | `ADR-003-localization-scope.md`, locale matrix and pseudo-localization fixture. |
| DG-06 | What is the web/desktop persistence support level? | Product says multi-platform. | Drift storage/runtime behavior differs across native, web and desktop. | Select and test one database opener per platform; document unsupported plugin capabilities. | `ADR-004-local-storage-platform-matrix.md` and platform smoke tests. |
...
| `lib/domain/` | Pure business model | entities, value objects, repository ports, use cases, policies, Study Mode contract/factory | Flutter, Riverpod, Drift, data or presentation imports |
...
## 6. Quy trình bàn giao chuẩn cho từng feature
...
| --- | --- | --- | --- | --- |
| 0.1 CP | Resolve DG-01 Deck model | M | — | Approved ADR; business/design prose and fixtures agree; mixed-content decision table exists. |
| 0.2 CP | Resolve DG-02 platform/adaptive scope | M | — | Supported platform/window/input matrix and parity policy approved. |
| 0.3 CP | Freeze SRS Policy v1 | M | — | Implement đúng `leitner-8-box-v1`, decision rows SRS8-001..016, UTC clock và deterministic contract tests; không thêm ease/SM-2/FSRS. |
...
| 0.5 CP | Establish documentation set | M | 0.1–0.4 | `docs/architecture`, `docs/decision-tables`, `docs/business/navigation`, `docs/database`, traceability register and ADR index. |
...
| 2.6 CP | Responsive foundation | XL | 2.2–2.5 | ScreenInfo/Class, compact-mobile density, adaptive family values, content width caps, grid/pane rules and resize tests. |
| 2.7 CP | Theme extensions/accessors | L | 2.6 | Dimension, semantic color/text and context accessors; token-only core with no Riverpod. |
| 2.8 CP | Theme assembly | L | 2.7 | Material 3 light/dark ThemeData, component themes, system UI appearance and runtime theme mode. |
| 2.9 | High-contrast readiness | M | 2.8, 0.6 | Either implemented and tested or explicitly deferred with no false support claim. |
| 2.10 CP | Foundation contract tests | L | 2.2–2.8 | Token-value tests, theme completeness, no raw value scans and responsive snapshots at all width classes. |
...
| 3.7 CP | Feedback primitives | XL | 3.2–3.5 | Progress, banner, snackbar service, loading/skeleton, empty/error/offline, dialog, sheet and menu. |
...
| 3.10 CP | Shared composites | XL | 3.2–3.9 | ConfirmDialog, SelectSheet, StatusCardRow, ActionCallout, breadcrumb and study prompt patterns; feature-free APIs. |
| 3.11 CP | Component golden/a11y catalog | XL | 3.1–3.10 | Variant × state × theme matrix, 390 baseline goldens, text-scale/focus/semantics tests, no open P0/P1. |
...
| WBS | Work package | Size | Depends on | Deliverable / Definition of Done |
| --- | --- | --- | --- | --- |
| 4.1 CP | Database runtime | L | 1.1, 1.6 | Drift database opened correctly on each supported platform, off main isolate where applicable, with close/lifecycle tests. |
...
| 4.8 CP | App DI graph | L | 4.1–4.7 | Keep-alive generated providers for DB/DAO/repos/services; startup fail-fast graph test. |
...
| --- | --- | --- | --- | --- |
| 5.1.1 CP | Language Pair domain/data | L | 4.8 | Create/select/list, duplicate normalization, stable identity and Deck dependency guard. |
| 5.1.2 CP | First-run language UI | L | 3.11, 5.1.1 | Two required selectors, search/long-name/validation/save-failure/draft states in en/vi. |
...
| 5.2.3 CP | First-run landing + two-step setup | XL | 3.11, 5.1.2, 5.2.2 | Landing/Not-now/import handoff, step drafts, submitting/failure/success/callout states. |
...
| 5.3.2 CP | Card Editor | XL | 3.11, 5.3.1 | Default/validation/duplicate/submitting/failure/dirty-discard/stale-target/keyboard states. |
...
| 5.4.3 CP | Leitner 8 Box policy implementation | XL | 0.3, 5.4.1 | Pure domain policy: Box 0 activation; Box 1..7 intervals 1/3/7/14/30/60/120; Box 8 mastered; correct +1, sticky-wrong -1. |
...
| 5.6.4 CP | Shared study shell | L | 3.11 | Constrained progress/header/prompt/action/error composition across all stages and widths. |
| 5.6.5 CP | Review UI | L | 5.6.3–5.6.4 | Browse/audio/edit handoff/loading/error/end states. |
| 5.6.6 CP | Match UI | XL | 5.6.3–5.6.4 | Playing/selected/correct/wrong/almost/round-complete/retry states with non-color cues. |
| 5.6.7 CP | Guess UI | L | 5.6.3–5.6.4 | Exactly five choices, one correct, invalid-pool recovery and long-text states. |
| 5.6.8 CP | Recall UI | XL | 5.6.3–5.6.4 | Reveal, remembered/forgot, injected 20s timer, pause/resume and deadline/tap race behavior. |
| 5.6.9 CP | Fill UI | XL | 5.6.3–5.6.4 | Keyboard, hint, compare, correct/wrong/retry and multilingual input states. |
| 5.6.10 CP | Answer persistence/recovery | XL | 5.6.5–5.6.9 | Attempt saved before advance; retry same key; stale writer blocked; answer-save-error retains evidence. |
| 5.6.11 CP | Mastery rounds and relearn | XL | 5.6.10 | Failed set deduped, unlimited retry rounds, separate persistence retry/relearn namespaces. |
...
| 5.6.14 CP | Study test pyramid | XL | 5.6.2–5.6.13 | Contract, repository, provider, fake-clock, widget/golden and interruption/restart E2E tests. |
...
| 5.7.2 CP | Today states | XL | 3.11, 5.7.1 | Empty library, not studied, due, paused, caught-up, loading/error/offline and create sheet. |
| 5.7.3 CP | Start/continue handoffs | L | 5.6.12, 5.7.2 | Revalidates before navigation; paused session outranks new review. |
| 5.7.4 CP | First-learning release E2E | XL | 5.7.3 | Fresh install → 5 valid cards → all modes → result → due state; force-close/resume and offline variants. |
| 5.7.5 CP | Release gate | L | 5.7.4 | Full verifier passes; no audit P0/P1; canonical state parity and platform smoke evidence attached. |
...
| --- | --- | --- | --- | --- |
| 6.1 | Deck metadata/lifecycle | XL | 5.7 | edit, move, delete, reset progress with impact/confirm/recovery. |
...
| WBS | Complete feature slice | Size | Depends on | Completion boundary |
| --- | --- | --- | --- | --- |
| 8.1 | Appearance preference | M | 2.8 | System/light/dark globally applied and persisted; invalid fallback. |
| 8.2 | Study/SRS preferences | L | 0.3, 5.6 | Defaults validated and snapshotted only into new sessions. |
| 8.3 | Mode preferences | L | 5.5 | Availability/default/order constraints without breaking required stages. |
...
| 9.3 | Notification entry | M | 5.7, 9.2 | Tap navigates to current learning entry and revalidates; never auto-starts Study. |
...
| WBS | Complete feature slice | Size | Depends on | Completion boundary |
| --- | --- | --- | --- | --- |
| 11.1 | Versioned statistics projection | XL | 5.4, 7.2 | No double-count, parent aggregation, formula/time-range versions and rebuild. |
...
| WBS | Work package | Size | Depends on | Deliverable / Definition of Done |
| --- | --- | --- | --- | --- |
| 16.1 | Full responsive/platform certification | XL | selected release waves | Android/iOS/web/Windows/macOS/Linux support matrix; resize, orientation, keyboard/mouse and state restoration. |
| 16.2 | Accessibility certification | XL | selected release waves | Semantics, screen reader, keyboard, focus, contrast, text 200%, reduced motion; KIT-42 sign-off. |
| 16.3 | Localization certification | L | selected release waves | en/vi completeness, plural/date/number formatting, expansion/CJK fixtures and no hardcoded UI copy. |
| 16.4 | Performance/reliability budgets | XL | selected release waves | Startup, frame, DB/query, memory, battery/background and large-library budgets with repeatable benchmarks. |
| 16.5 | Privacy/security review | XL | selected release waves | Data inventory, secret handling, logs, import/backup threats, permissions, retention and deletion behavior. |
| 16.6 | Migration/upgrade rehearsal | L | selected release waves | Upgrade from every released schema, rollback/recovery drill and restore fixture validation. |
| 16.7 | Release observability/support | L | selected release waves | Consent-aware crash reporting decision, redaction, diagnostics export and support runbook. |
