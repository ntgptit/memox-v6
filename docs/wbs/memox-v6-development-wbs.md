...
| MX-VIS-044 | Deck detail — leaf branch | Error | `flashcard-list--error` | Composition built, **not yet measured**. Same centred failure as `MX-VIS-038` with the leaf's own title (`Couldn’t load cards`), replacing the inline banner. Needs a fixture that breaks the card read plus a spec |
