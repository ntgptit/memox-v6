...
| MX-VIS-064 | Match | Correct | `match-mode--correct` | **Measured 16.00% light / 11.57% dark — blocked on the tile tone map.** The kit's `Tile.jsx` tints correct with `success-soft` and a success border, and hides a *matched* tile entirely (`visibility: hidden`); this build paints `MxCardVariant.primary`, a solid brand fill — sampled (34,31,53) against the kit's (237,237,239). Closing it needs soft success/error tones on the tile and a matched tile that leaves the board |
| MX-VIS-065 | Match | Wrong | `match-mode--wrong` | **Measured 12.07% light / 4.03% dark — same tone map as `MX-VIS-064`**; the kit tints wrong with `error-soft` |
